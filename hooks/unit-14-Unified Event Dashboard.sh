...
# OPTIMIZATION: Try direct UUID-based lookup first (FAST PATH), fall back to search (SLOW PATH)
_find_jsonl() {
  local uuid="$1"

  # FAST PATH: Try direct match on UUID filename (most sessions use UUID as filename)
  local direct_match
  direct_match=$(find "$PROJECT_DIR" -name "${uuid}.jsonl" -type f 2>/dev/null | head -1)
  if [[ -n "$direct_match" ]]; then
    echo "$direct_match"
    return 0
  fi

  # SLOW PATH: Search all JSONL files if direct match failed
  # (this is for legacy sessions or unusual directory structures)
  local best_jsonl=""
  local best_count=0
...
_sense_generation() {
  ...
  # OPTIMIZATION: Use awk instead of Python
  local generation=0
  local current_parent
  current_parent=$(grep "\"sessionId\".*\"$uuid\"" "$jsonl" 2>/dev/null | head -1 | \
    awk '{
      if (match($0, /"parentUuid"[[:space:]]*:[[:space:]]*null/)) {
        print "null"; exit
      } else if (match($0, /"parentUuid"[[:space:]]*:[[:space:]]*"([^"]+)"/, arr)) {
  ...
  [[ -z "$current_parent" ]] && current_parent="?"
...
_sense_model() {
  ...
  # Ground truth: Read model from session JSONL message.model field
  # OPTIMIZATION: Use awk instead of Python, pass JSONL as parameter instead of find
  if [[ -n "$uuid" && -n "$jsonl" ]]; then
    local model
    ...
    if [[ -n "$model" ]]; then
      echo "$model|GROUND_TRUTH|Session JSONL message.model field"
      return
    fi
  fi
...
_sense_qc_level() {
  ...
  if [[ -n "$loa_cap" ]]; then
    # Map LOA_CAP (Sheridan-Verplank scale: 0-10) to QC_LEVEL enums
    # LOA 2 (suggest, not act) → QC0_HUMAN_ONLY
    # LOA 4 (designed actions) → QC1_SUPERVISED
    # LOA 6 (imprinted autonomy) → QC2_FULLY_AUTONOMOUS
    case "$loa_cap" in
...
" 2>/dev/null)
  ...
  # Ground truth: JSONL line count (OPTIMIZATION: use awk instead of grep -c)
  local line_count
  line_count=$(awk -v uuid="$uuid" '/"sessionId"[[:space:]]*:[[:space:]]*"'"$uuid"'"/ { count++ } END { print count+0 }' "$jsonl" 2>/dev/null) || line_count=0
...
_build_identity() {
  ...
  # Find JSONL (CACHED once at start)
  local jsonl
  ...
  # OPTIMIZATION: Parallelize independent sensors (_sense_avatar and _sense_location don't need JSONL)
  # Launch async background jobs for JSONL-independent sensors
  ...
  # Serial sensors that depend on JSONL (can't parallelize without more infrastructure)
